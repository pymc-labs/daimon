...
async def _rename_thread_impl(  # pyright: ignore[reportUnusedFunction]
    ...
    *,
    ...
    ``_check_rename_thread_permission``. An archived thread is unarchived
    first (Discord refuses every other edit while archived, and only a
    manage_threads holder gets this far with one). Returns the thread as
    Discord reports it after the edit."""
    validated_name = _validate_thread_name(name)
    ...
    async with rest_client(token) as c:
        ...
        try:
            ...
            raise ToolError(f"discord refused the rename: {e.text}") from e
