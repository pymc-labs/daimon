...
Handler contract:
  ...
  handle_agent_setup_action(runtime, payload)
    ...
    the boundary catch, which renders into the open view rather than failing
    silently.

The three read-only panel views (Agents, Details, Who answers where) are
dispatched first, by ``PANEL_ACTION_IDS``, and read their state from the typed
``PanelMetadata`` the views carry. Navigation pushes; paging and the Details
expansions update the view they were clicked on, so the stack never grows past
the two depths the design uses. They open no editor: the only write reachable
from them is the New agent form, and the coding-tools button, which mints a
token behind a live admin check. Everything below them is the legacy editor
surface, still reachable by its own action ids until it is deleted.
...
from daimon.adapters.slack.agent_setup import gate, panel_views
...
async def handle_agent_setup_action(runtime: SlackRuntime, payload: dict[str, Any]) -> None:
    ...
    # The three panel views carry their own typed metadata; a legacy editor
    # view decodes to None here and falls through to the dict below.
    panel_meta = decode_panel_metadata(raw_meta)

    team_id = team_id or (panel_meta.team_id if panel_meta else "") or meta.get("team_id") or ""
    channel_info: dict[str, Any] = payload.get("channel") or {}
    channel_id = str(
        (panel_meta.channel_id if panel_meta else "")
    ...
    try:
        ...
        # A panel action id on a legacy view (`agent_setup__new` is on both)
        # falls through to the legacy chain below rather than being dropped.
