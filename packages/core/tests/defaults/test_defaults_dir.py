...
def test_defaults_ships_dev_agent_with_copilot_mcp() -> None:
    ...
    The Sonnet family is deliberate — an earlier revision ran claude-opus-4-8
    and was moved off it. This optional agent does not follow the main default.
    """
    ...
    assert dev.model == "claude-sonnet-5", (
        f"dev_agent pins {dev.model}; expected the deliberate Sonnet 5 model"
    )
