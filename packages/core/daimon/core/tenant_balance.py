"""Per-tenant balance gate + debit math. TOPUP-01 (D-14/D-15).

Admission (D-14): balance > 0. Composed with is_over_cap by the caller —
...
def debit_amount(cost: float | None, *, markup: Decimal) -> Decimal:
    ...
    Returns Decimal quantized to 6 decimal places (sub-cent turn debits, D-17).
    """
...
async def is_over_balance(
    *,
    ...
    """True iff the tenant balance is depleted (<= 0). D-14.
    ...
    Exceptions propagate (fail-closed admission, D-24).
    """
